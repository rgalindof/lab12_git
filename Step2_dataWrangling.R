...
cpm <- cpm(myDGEList) 
colSums(cpm)
log2.cpm <- cpm(myDGEList, log=TRUE) #default is log2, so no need to specify
...
keepers <- rowSums(cpm>1)>=5 #in this case 5 was chosen because it was the n of the smallest group
# now use base R's simple subsetting method to filter your DGEList based on the logical produced above
#subsetting works as follows: [rows, columns], so anything you want to do to the rows you put on the left, and everything you want to do to columns you put on the right.
#subsetting will work with lists and matrix
...
# take a look at this new DGEList object...how has it changed? ans: the normalization factors (norm.factors) are now not all 1. some are tweaked up a bit and some are tweaked down.
# the method used was trimmed mean of m-values to carry out between-sample normalization
...
sampleLabels <- targets$sample
myDGEList <- DGEList(human_covid_data_lung_biopsy)
log2.cpm <- cpm(myDGEList, log=TRUE)

log2.cpm.df <- as_tibble(log2.cpm, rownames = "geneID")
colnames(log2.cpm.df) <- c("geneID", sampleLabels)
log2.cpm.df.pivot <- pivot_longer(log2.cpm.df, # dataframe to be pivoted
                                  cols = Series15_HealthyLungBiopsy_2:Series15_COVID19Lung_1, # column names to be stored as a SINGLE variable
                                  names_to = "samples", # name of that new variable (column)
                                  values_to = "expression") # name of new variable (column) storing all the values (data)
