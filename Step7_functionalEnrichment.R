...
myghostplot <- gostplot(gost.res, interactive = F, capped = T) #set interactive=FALSE to get plot for publications
...
publish_gostplot(
  myghostplot, #your static gostplot from above
...
gost.res.mod <- gost(rownames(myModule_up), organism = "hsapiens", correction_method = "fdr")
myghostplot.mod <- gostplot(gost.res, interactive = F, capped = T)
myghostplot.mod <- gostplot(gost.res, interactive = T, capped = T)
...
c2cp <- read.gmt("c2.cp.v7.4.symbols.gmt")
...
# now ready to run GSEA using the 'GSEA' function from clusterProfiler
...
gseaplot2(myGSEA.res, 
          geneSetID = c(14,56,296), #can choose multiple signatures to overlay in this plot
          pvalue_table = FALSE, #can set this to FALSE for a cleaner plot
          title = myGSEA.res$Description[56]) #can also turn off this title
...
broadSet.C2.ALL <- getGmt("c2.all.v7.4.symbols.gmt", geneIdType=SymbolIdentifier())
#extract as a list
