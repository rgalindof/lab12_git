...
Txi_gene <- tximport(path, 
                     type = "kallisto", 
                     tx2gene = Tx, #transcript to gene mapping (what we just made called Tx)
                     txOut = FALSE, #How does the result change if this =FALSE = collapse to gene vs =TRUE=don't collapse to gene (we want transcript)
...
Txi_gene <- tximport(path, 
                     type = "kallisto", 
                     tx2gene = Tx, 
                     txOut = FALSE, #determines whether your data represented at transcript or gene level. FALSE = genelevel. TRUE = transcriptlevel
                     countsFromAbundance = "lengthScaledTPM",
                     ignoreTxVersion = TRUE)
