...
#note: we use Spearman, instead of Pearson, for clustering samples because it gives equal weight to highly vs lowly expressed transcripts or genes. Pearson first clusters data, but Spearman will first take a rnak ordering of that data and operate on the ranks to figure out the correlation.
#thus, Spearman is less sensitive to magnitude. You don't want a very highly expressed gene to skew the clustering. Thus, you use Spearman correlation for samples, but Pearson correlation for genes.
...
#simplified heatmap grouped by healthy/disease
...
# modulePick <- 2 chooses ONLY the genes that went UP with infection. <- 1 chooses ONLY the genes that went down with infection.
